...
import numpy as np
import sys
# todo: display accuracy, make calibration slow
...
if __name__ == "__main__":
    ...
    for Ttype in [True,True,True,True,True]: #loop for each "trial"
        ...
        #gc.disable()
##        a=0;b=0;
##        t0=clock.getTime(); N=1000;k=0
##        eyeTracker.getFixation()
##        D=np.ones((N,7))*-999
##        for i in range(N):
##            smp=eyeTracker.getGazePosition(units='deg',eyes=1)
##            #print smp, int(clock.getTime()*1000)
##            if smp!=None: eyeTracker.computeFixation(smp[:2])
##            b+=1
##            
##            if smp==None: a+=1
##            else:
##                fc,isFix=eyeTracker.getCurrentFixation()
##                D[k,:2]=smp[:2]
##                
##                D[k,4]=eyeTracker.fixdur
##                D[k,5]=isFix
##                if isFix: D[k,2:4]=fc
##                else: D[k,2:4]=np.nan
##                D[k,6]=smp[2]
...
##            continue
##            if smp==None:
##                point.setPos([0,0],units='pix')
##                point2.setPos([0,0],units='pix')
##            else:
##                if not np.isnan(smp[1]): point.setPos(smp[:2],units='pix')
##                if not np.isnan(smp[2]): point2.setPos(smp[2:],units='pix')
##            
##            point.draw()
##            point2.draw()
##            win.flip()
##        D=D[:k,:] 
##        print (t0-clock.getTime())*1000/float(N),'ms ', a/float(b)
##        win.flip()
##        core.wait(1)
